...
var ChunkCache = (_class3 = class {constructor() { _class3.prototype.__init5.call(this);_class3.prototype.__init6.call(this);_class3.prototype.__init7.call(this);_class3.prototype.__init8.call(this);_class3.prototype.__init9.call(this); }
  __init5() {this.index = 0}
  // Index >= 0 means local xorb, < 0 means remote xorb
  __init6() {this.xorbIndices = new Int32Array(CHUNK_CACHE_INITIAL_SIZE)}
  // Max 8K chunks per xorb, less than 64K uint16_t
  __init7() {this.chunkIndices = new Uint16Array(CHUNK_CACHE_INITIAL_SIZE)}
  ...
  __init9() {this.hmacs = /* @__PURE__ */ new Set()}
...
function backtrackDedup(xorb, computeHmac, shardData, chunkCache, chunkMetadata, dedupedBytes) {
  ...
  for (let chunkToRecheckIndex = _nullishCoalesce(_optionalChain([xorb, 'access', _74 => _74.immutableData, 'optionalAccess', _75 => _75.chunkIndex]), () => ( 0)); chunkToRecheckIndex < xorb.chunks.length; chunkToRecheckIndex++) {
  ...
  try {
    ...
    const processChunks = async (chunkData) => {
      for (const chunk2 of chunkData) {
        chunksProcessed++;
        if (_optionalChain([opts, 'optionalAccess', _76 => _76.isAtBeginning]) && chunksProcessed === 1) {
          chunk2.dedup = true;
        }
    ...
    while (true) {
      if (_optionalChain([opts, 'optionalAccess', _77 => _77.end]) !== void 0 && totalBytes >= opts.end) {
        break;
      }
      if (_optionalChain([opts, 'optionalAccess', _78 => _78.maxChunks]) !== void 0 && chunksProcessed >= opts.maxChunks) {
        break;
      }
      const { done, value } = await reader.read();
      if (done) {
...
async function* commitIter(params) {
  ...
  if (useXet) {
    const info = await (_nullishCoalesce(params.fetch, () => ( fetch)))(
      `${_nullishCoalesce(params.hubUrl, () => ( HUB_URL))}/api/${repoId.type}s/${repoId.name}?expand[]=xetEnabled`,
      {
        headers: {
          ...accessToken && { Authorization: `Bearer ${accessToken}` }
        }
  ...
  try {
    const allOperations = (await Promise.all(
      params.operations.map(async (operation) => {
        ...
        _optionalChain([abortSignal, 'optionalAccess', _79 => _79.throwIfAborted, 'call', _80 => _80()]);
    ...
    const gitAttributes = _optionalChain([allOperations, 'access', _81 => _81.filter, 'call', _82 => _82(isFileOperation), 'access', _83 => _83.find, 'call', _84 => _84((op) => op.path === ".gitattributes"), 'optionalAccess', _85 => _85.content]);
    for (const operations of chunk(allOperations.filter(isFileOperation), 100)) {
      ...
      _optionalChain([abortSignal, 'optionalAccess', _86 => _86.throwIfAborted, 'call', _87 => _87()]);
    ...
    for (const operations of chunk(
      ...
      _optionalChain([abortSignal, 'optionalAccess', _88 => _88.throwIfAborted, 'call', _89 => _89()]);
      const payload = {
        operation: "upload",
      ...
      if (useXet) {
        for (const obj of json.objects) {
          ...
          if (!_optionalChain([obj, 'access', _90 => _90.actions, 'optionalAccess', _91 => _91.upload])) {
            yield {
              event: "fileProgress",
              path: op.path,
              progress: 1,
        ...
        const source = async function* () {
          for (const obj of json.objects) {
            const op = shaToOperation.get(obj.oid);
            if (!op || !_optionalChain([obj, 'access', _92 => _92.actions, 'optionalAccess', _93 => _93.upload])) {
              continue;
            }
        ...
        yield* eventToGenerator((yieldCallback, returnCallback, rejectCallback) => {
          return promisesQueueStreaming(
            json.objects.map((obj) => async () => {
              ...
              if (_optionalChain([header, 'optionalAccess', _100 => _100.chunk_size])) {
                ...
                await promisesQueueStreaming(
                  parts.map((part) => async () => {
                    _optionalChain([abortSignal, 'optionalAccess', _101 => _101.throwIfAborted, 'call', _102 => _102()]);
                ...
                _optionalChain([abortSignal, 'optionalAccess', _103 => _103.throwIfAborted, 'call', _104 => _104()]);
    ...
    _optionalChain([abortSignal, 'optionalAccess', _105 => _105.throwIfAborted, 'call', _106 => _106()]);
...
async function* listDatasets(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _107 => _107.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _108 => _108.search, 'optionalAccess', _109 => _109.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _110 => _110.search, 'optionalAccess', _111 => _111.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _112 => _112.search, 'optionalAccess', _113 => _113.tags, 'optionalAccess', _114 => _114.map, 'call', _115 => _115((tag) => ["filter", tag])]), () => ( [])),
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _116 => _116.additionalFields, 'optionalAccess', _117 => _117.map, 'call', _118 => _118((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _119 => _119.hubUrl]) || HUB_URL}/api/datasets` + (search ? "?" + search : "");
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _120 => _120.fetch]), () => ( fetch)))(url, {
      headers: {
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _121 => _121.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
...
async function datasetInfo(params) {
  ...
  const search = new URLSearchParams([
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _122 => _122.additionalFields, 'optionalAccess', _123 => _123.map, 'call', _124 => _124((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _125 => _125.hubUrl]) || HUB_URL}/api/datasets/${params.name}/revision/${encodeURIComponent(
  ...
  return {
    ..._optionalChain([params, 'optionalAccess', _126 => _126.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
...
async function fileDownloadInfo(params) {
  ...
  let size;
  let xetInfo;
  if (_optionalChain([resp, 'access', _127 => _127.headers, 'access', _128 => _128.get, 'call', _129 => _129("Content-Type"), 'optionalAccess', _130 => _130.includes, 'call', _131 => _131("application/vnd.xet-fileinfo+json")])) {
  ...
  return {
    ...
    url: resp.url && (new URL(resp.url).origin === new URL(hubUrl).origin || _optionalChain([resp, 'access', _132 => _132.headers, 'access', _133 => _133.get, 'call', _134 => _134("X-Cache"), 'optionalAccess', _135 => _135.endsWith, 'call', _136 => _136(" cloudfront")])) ? resp.url : url
  };
}
...
async function* listModels(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _137 => _137.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _138 => _138.search, 'optionalAccess', _139 => _139.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _140 => _140.search, 'optionalAccess', _141 => _141.task]) ? { pipeline_tag: params.search.task } : void 0,
      ..._optionalChain([params, 'optionalAccess', _142 => _142.search, 'optionalAccess', _143 => _143.query]) ? { search: params.search.query } : void 0,
      ..._optionalChain([params, 'optionalAccess', _144 => _144.search, 'optionalAccess', _145 => _145.inferenceProviders]) ? { inference_provider: params.search.inferenceProviders.join(",") } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _146 => _146.search, 'optionalAccess', _147 => _147.tags, 'optionalAccess', _148 => _148.map, 'call', _149 => _149((tag) => ["filter", tag])]), () => ( [])),
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _150 => _150.additionalFields, 'optionalAccess', _151 => _151.map, 'call', _152 => _152((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _153 => _153.hubUrl]) || HUB_URL}/api/models?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _154 => _154.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      ...
      if (_optionalChain([params, 'optionalAccess', _155 => _155.additionalFields, 'optionalAccess', _156 => _156.includes, 'call', _157 => _157("inferenceProviderMapping")]) && item.inferenceProviderMapping) {
      ...
      yield {
        ..._optionalChain([params, 'optionalAccess', _158 => _158.additionalFields]) && pick(normalizedItem, params.additionalFields),
        id: item._id,
        name: item.id,
        private: item.private,
...
async function* listSpaces(params) {
  ...
  const search = new URLSearchParams([
    ...Object.entries({
      ...
      ..._optionalChain([params, 'optionalAccess', _161 => _161.search, 'optionalAccess', _162 => _162.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _163 => _163.search, 'optionalAccess', _164 => _164.tags, 'optionalAccess', _165 => _165.map, 'call', _166 => _166((tag) => ["filter", tag])]), () => ( [])),
    ...[...SPACE_EXPAND_KEYS, ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _167 => _167.additionalFields]), () => ( []))].map(
      (val) => ["expand", val]
    )
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _168 => _168.hubUrl]) || HUB_URL}/api/spaces?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _169 => _169.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _170 => _170.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
        sdk: item.sdk,
        likes: item.likes,
...
async function* listCollections(params) {
  ...
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _171 => _171.limit]), () => ( Infinity));
  searchParams.append("limit", String(Math.min(totalToFetch, 100)));
  if (_optionalChain([params, 'optionalAccess', _172 => _172.sort])) {
    searchParams.append("sort", params.sort);
  }
  if (_optionalChain([params, 'optionalAccess', _173 => _173.search, 'optionalAccess', _174 => _174.owner])) {
    for (const owner of params.search.owner) {
      searchParams.append("owner", owner);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _175 => _175.search, 'optionalAccess', _176 => _176.item])) {
    for (const item of params.search.item) {
      searchParams.append("item", item);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _177 => _177.search, 'optionalAccess', _178 => _178.q])) {
    searchParams.append("q", params.search.q);
  }
  let url = `${_optionalChain([params, 'optionalAccess', _179 => _179.hubUrl]) || HUB_URL}/api/collections?${searchParams}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _180 => _180.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
...
async function modelInfo(params) {
  ...
  const search = new URLSearchParams([
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _181 => _181.additionalFields, 'optionalAccess', _182 => _182.map, 'call', _183 => _183((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _184 => _184.hubUrl]) || HUB_URL}/api/models/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  if (_optionalChain([params, 'optionalAccess', _185 => _185.additionalFields, 'optionalAccess', _186 => _186.includes, 'call', _187 => _187("inferenceProviderMapping")]) && data.inferenceProviderMapping) {
    normalizedData.inferenceProviderMapping = normalizeInferenceProviderMapping(data.id, data.inferenceProviderMapping);
  }
  return {
    ..._optionalChain([params, 'optionalAccess', _188 => _188.additionalFields]) && pick(normalizedData, params.additionalFields),
    id: data._id,
    name: data.id,
    private: data.private,
...
async function oauthHandleRedirect(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _189 => _189.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _190 => _190.nonce]) || !_optionalChain([opts, 'optionalAccess', _191 => _191.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const redirectedUrl = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _192 => _192.redirectedUrl]), () => ( window.location.href));
  const searchParams = (() => {
    try {
  ...
  const nonce = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _193 => _193.nonce]), () => ( localStorage.getItem("huggingface.co:oauth:nonce")));
  if (!code) {
  ...
  const codeVerifier = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _194 => _194.codeVerifier]), () => ( localStorage.getItem("huggingface.co:oauth:code_verifier")));
  if (!codeVerifier) {
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _195 => _195.hubUrl]) || HUB_URL;
  ...
  if (!_optionalChain([opts, 'optionalAccess', _196 => _196.codeVerifier])) {
    localStorage.removeItem("huggingface.co:oauth:code_verifier");
  }
  if (!_optionalChain([opts, 'optionalAccess', _197 => _197.nonce])) {
...
async function oauthHandleRedirectIfPresent(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _198 => _198.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _199 => _199.nonce]) || !_optionalChain([opts, 'optionalAccess', _200 => _200.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const searchParams = new URLSearchParams(_nullishCoalesce(_optionalChain([opts, 'optionalAccess', _201 => _201.redirectedUrl]), () => ( window.location.search)));
...
async function oauthLoginUrl(opts) {
  if (typeof window === "undefined" && (!_optionalChain([opts, 'optionalAccess', _202 => _202.redirectUrl]) || !_optionalChain([opts, 'optionalAccess', _203 => _203.clientId]))) {
    throw new Error("oauthLogin is only available in the browser, unless you provide clientId and redirectUrl");
  }
  if (typeof localStorage === "undefined" && !_optionalChain([opts, 'optionalAccess', _204 => _204.localStorage])) {
    throw new Error(
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _205 => _205.hubUrl]) || HUB_URL;
  const openidConfigUrl = `${new URL(hubUrl).origin}/.well-known/openid-configuration`;
  const openidConfigRes = await fetch(openidConfigUrl, {
    headers: {
      Accept: "application/json"
  ...
  if (_optionalChain([opts, 'optionalAccess', _206 => _206.localStorage])) {
  ...
  const redirectUri = _optionalChain([opts, 'optionalAccess', _207 => _207.redirectUrl]) || (typeof window !== "undefined" ? window.location.href : void 0);
  if (!redirectUri) {
  ...
  const state = JSON.stringify({
    nonce: newNonce,
    redirectUri,
    state: _optionalChain([opts, 'optionalAccess', _208 => _208.state])
  });
  const variables = (
    // @ts-expect-error window.huggingface is defined inside static Spaces.
    typeof window !== "undefined" ? _nullishCoalesce(_optionalChain([window, 'access', _209 => _209.huggingface, 'optionalAccess', _210 => _210.variables]), () => ( null)) : null
  );
  const clientId = _optionalChain([opts, 'optionalAccess', _211 => _211.clientId]) || _optionalChain([variables, 'optionalAccess', _212 => _212.OAUTH_CLIENT_ID]);
  if (!clientId) {
    if (variables) {
  ...
  return `${opendidConfig.authorization_endpoint}?${new URLSearchParams({
    client_id: clientId,
    scope: _optionalChain([opts, 'optionalAccess', _213 => _213.scopes]) || _optionalChain([variables, 'optionalAccess', _214 => _214.OAUTH_SCOPES]) || "openid profile",
    response_type: "code",
...
async function parseSafetensorsMetadata(params) {
  ...
  const quantConfig = _optionalChain([modelConfig, 'optionalAccess', _215 => _215.quantization_config]);
  if (params.path && RE_SAFETENSORS_FILE.test(params.path) || await fileExists({ ...params, path: SAFETENSORS_FILE })) {
    ...
    return {
      ...
      ...params.computeParametersCount ? {
        ...
        parameterTotal: (
          /// shortcut: get param count directly from metadata
          _optionalChain([index, 'access', _216 => _216.metadata, 'optionalAccess', _217 => _217.total_parameters]) ? typeof index.metadata.total_parameters === "number" ? index.metadata.total_parameters : typeof index.metadata.total_parameters === "string" ? parseInt(index.metadata.total_parameters) : void 0 : void 0
        )
      } : void 0
    };
  } else {
...
function getQuantizationMultiplier(tensorName, dtype, quantConfig) {
  ...
  const quantMethod = _optionalChain([quantConfig, 'access', _218 => _218.quant_method, 'optionalAccess', _219 => _219.toLowerCase, 'call', _220 => _220()]);
  switch (quantMethod) {
    case "mxfp4":
...
function shouldSkipTensor(tensorName, quantConfig) {
  ...
  const quantMethod = _optionalChain([quantConfig, 'access', _221 => _221.quant_method, 'optionalAccess', _222 => _222.toLowerCase, 'call', _223 => _223()]);
...
async function spaceInfo(params) {
  ...
  const search = new URLSearchParams([
    ...
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _224 => _224.additionalFields, 'optionalAccess', _225 => _225.map, 'call', _226 => _226((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _227 => _227.hubUrl]) || HUB_URL}/api/spaces/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  const data = await response.json();
  return {
    ..._optionalChain([params, 'optionalAccess', _228 => _228.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
    sdk: data.sdk,
    likes: data.likes,
...
async function* uploadFilesWithProgress(params) {
  return yield* commitIter({
    ...
    fetch: async (input, init) => {
      ...
      _optionalChain([init, 'access', _229 => _229.signal, 'optionalAccess', _230 => _230.throwIfAborted, 'call', _231 => _231()]);
      xhr.send(init.body);
      return new Promise((resolve2, reject) => {
        ...
        if (init.signal) {
          init.signal.addEventListener("abort", () => {
            xhr.abort();
            try {
              _optionalChain([init, 'access', _232 => _232.signal, 'optionalAccess', _233 => _233.throwIfAborted, 'call', _234 => _234()]);
            } catch (err) {
              reject(err);
            }
          });
...
async function whoAmI(params) {
  ...
  if (typeof _optionalChain([response, 'access', _235 => _235.auth, 'access', _236 => _236.accessToken, 'optionalAccess', _237 => _237.createdAt]) === "string") {
