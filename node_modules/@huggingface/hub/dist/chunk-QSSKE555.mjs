...
async function scanCachedRepo(repoPath) {
  ...
  return {
    ...
    size: [...blobStats.values()].reduce((sum2, stat3) => sum2 + stat3.size, 0),
...
async function getWebWorkerCode() {
  const sha256Module = await import("./sha256-wrapper-6SHY2YLK.mjs");
...
async function* sha256(buffer, opts) {
  ...
  if (isFrontend) {
    ...
    if (!wasmModule) {
      wasmModule = await import("./sha256-wrapper-6SHY2YLK.mjs");
    }
  ...
  if (!cryptoModule) {
    cryptoModule = await import("./sha256-node-KI7ZLGUH.mjs");
  }
...
async function createBlobs(url, destPath, opts) {
  ...
  if (url.protocol === "file:") {
    const { FileBlob } = await import("./FileBlob-7KXBO7HQ.mjs");
    const { subPaths } = await import("./sub-paths-JTJURQDM.mjs");
...
async function* createXorbs(fileSources, params) {
  const chunkModule = await import("./chunker_wasm-77LEM2OQ.mjs");
  let xorbId = 0;
